...
function calculateBalance() {
  ...
  const positions = new Array(weights.length).fill(null);
  ...
  // 7) sequential torque balance for items 2–6
  let torqueSum = weights[0] * (p0 - center);
  for (let i = 1; i < weights.length; i++) {
    const w = weights[i], wh = widths[i];
    if (w > 0 && wh > 0) {
      // di = −torqueSum / w_i
      const di = -torqueSum / w;
      let pi = center + di;
      // clamp pi into margins
      const half = wh/2;
      const minPi = m + half;
      const maxPi = L - m - half;
      if (pi < minPi || pi > maxPi) {
        const old = pi;
        pi = Math.min(Math.max(pi, minPi), maxPi);
        warn += (warn ? ' ' : '') + `⚠️ Obj ${i+1} clamped from ${old.toFixed(2)} to ${pi.toFixed(2)}`;
      }
      positions[i] = pi;
      torqueSum += w * (pi - center);
    }
  }
