...
function calculateBalance() {
  ...
  // 2) container length & margin (ft or m)
  let L = rawSize;
  if (lUnit === 'm') L *= 0.3048;
  const marginFt = 8 / 12;
  let m = marginFt;
  ...
  // 7) running torque sum about center
  let torqueSum = weights[0] * (p0 - center);

  // 8) sequential solve for each next object
  for (let i = 1; i < weights.length; i++) {
    const Wi = weights[i], wi = widths[i];
    if (Wi <= 0 || wi <= 0) continue;

    // desired offset di so W0*d0 + W1*d1 + … + Wi*di = 0
    // => di = −(torqueSum) / Wi
