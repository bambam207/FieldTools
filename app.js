...
function calcSling() {
  ...
  if(!hasHeight&&!hasLegs){
    [60,50,45,35].forEach(thetaDeg=>{
      ...
      const L1b=D1Base/Math.cos(θ),L2b=D2Base/Math.cos(θ),
            H1b=D1Base*Math.tan(θ),H2b=D2Base*Math.tan(θ),
...
function calcContainer(){
  const wUnit=document.getElementById('cbWeightUnit').value,
        lUnit=document.getElementById('cbLengthUnit').value,
        cType=parseFloat(document.getElementById('cbContainerType').value);
  const toMeters=v=>v*0.3048,toFeet=v=>v/0.3048,
        toLbsFromKg=v=>v*2.20462,toLbsFromUsTon=v=>v*2000,
        toLbsFromMTon=v=>v*2204.62,
        fromLbsToKg=v=>v/2.20462,fromLbsToUsTon=v=>v/2000,
        fromLbsToMTon=v=>v/2204.62;
  ...
  for(let i=1;i<=6;i++){
    const wRaw=parseFloat(document.getElementById(`cbLoad${i}W`).value),
          dRaw=parseFloat(document.getElementById(`cbLoad${i}D`).value);
    ...
    const posFt=(desiredCenterFt*(cumW+wLbs)-cumM)/wLbs,
          leftFt=posFt-(dRaw/2)*(lUnit==='m'?toFeet(1):1);
    cumW+=wLbs;cumM+=wLbs*posFt;
    const posDisp=(lUnit==='m'?toMeters(posFt):posFt).toFixed(2),
          leftDisp=(lUnit==='m'?toMeters(leftFt):leftFt).toFixed(2);
  ...
  if(cumW===0){resultDiv.textContent='No loads entered.';return;}
  const overallCGFt=cumM/cumW,
        cgDisp=(lUnit==='m'?toMeters(overallCGFt):overallCGFt).toFixed(2),
        totalLoadDisp=wUnit==='kg'?fromLbsToKg(cumW).toFixed(2):
  ...
  resultDiv.innerHTML=`
    ...
    (${((overallCGFt/cType)*100).toFixed(1)}% of length)
  `;
}